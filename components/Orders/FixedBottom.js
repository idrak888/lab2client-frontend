  ...
  return (
    <footer className={`${styles.FixedBottom} navbar navbar-dark fixed-bottom`}>
      <div className="container">
        ...
        <Link className={`${styles.btn} btn`} href={data ? { pathname: '/order', query: { id: data.id } } : ""}><span className={styles.text}>Place an Order</span> <i className="bi bi-arrow-right" style={{marginLeft: 3, marginRight: 3}}></i></Link>
      </div>
    </footer>
  );
};
